...
`custom-functions.yml` keeps TrChat's `Custom:` layout. Each rule supports `enabled`, `priority`, `pattern`, optional `text-filter`, and `display.text`, `display.hover`, `display.url`, or `display.copy`. Display text and hover lines accept MiniMessage, legacy `&` codes, `&x` hex codes, and `&#RRGGBB`. The default rules turn URLs, `QQ123456`, and 12-character `BV` IDs into clickable chat components. Run `/fxchat reload` after editing this file.
